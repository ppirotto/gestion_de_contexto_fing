...
public class DroolsMessageComposerBean implements DroolsMessageComposer {
	...
	public String send(String originalMessage) {
		...
		DroolsInputMessage droolsMsg = new DroolsInputMessage();
		droolsMsg.setType("PRUEBA");
		droolsMsg.setBody(originalMessage);

		// Create the request message
		RemoteMessage message = new RemoteMessage();
		message.setService(SERVICE).setOperation("receiveMessage")
				.setContent(droolsMsg);
		...
		try {
			...
			if (reply.isFault()) {
				...
				out.println("==================================");
				out.println(response + "CONCHAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
				out.println("==================================");
			}
		} catch (IOException e) {
